...
def test_set_new_data_simple_modifications():
    ...
    assert_array_equal(im_1.get_fdata()[:], np.ones([3, 3, 3]))
...
def test_modify_image_type_remove_nan():
    ...
    assert im_new.get_data_dtype() == "uint8"
    assert im.get_data_dtype() == "float64"
    assert np.nan not in im_new.get_fdata()
...
def test_modify_affine_transformation_replace_sform():
    ...
    new_im_sform_true = modify_affine_transformation(
        im,
        new_aff=aff_new,
    ...
    new_im_sform_false = modify_affine_transformation(
        im,
        new_aff=aff_new,
        multiplication_side="replace",
        q_form=False,
...
def test_remove_nan():
    data_ts = np.array(
        [
            [[0, 1, 2, 3], [4, 5, np.nan, 7], [8, 9, 10, np.nan]],
            [[12, 13, 14, 15], [16, np.nan, 18, 19], [20, 21, 22, 23]],
        ],
    )
    ...
    data_no_nan_expected = np.array(
        [[[0, 1, 2, 3], [4, 5, 0, 7], [8, 9, 10, 0]], [[12, 13, 14, 15], [16, 0, 18, 19], [20, 21, 22, 23]]],
    )

    assert_array_equal(im_no_nan.get_fdata(), data_no_nan_expected)
