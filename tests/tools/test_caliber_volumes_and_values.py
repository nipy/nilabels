...
from nilabels.tools.caliber.volumes_and_values import (
    get_num_voxels_from_labels_list,
    get_total_num_nonzero_voxels,
...
def test_get_values_below_labels_list():
    ...
    sky_a = cube_shape(
        omega, center=cube_a_anat[0], side_length=cube_a_anat[1], foreground_intensity=cube_a_anat[2], dtype=np.float32,
    )
    sky_a += cube_shape(
        omega, center=cube_b_anat[0], side_length=cube_b_anat[1], foreground_intensity=cube_b_anat[2], dtype=np.float32,
    )
    sky_a += cube_shape(
        omega, center=cube_c_anat[0], side_length=cube_c_anat[1], foreground_intensity=cube_c_anat[2], dtype=np.float32,
    )
    sky_a += cube_shape(
        omega, center=cube_d_anat[0], side_length=cube_d_anat[1], foreground_intensity=cube_d_anat[2], dtype=np.float32,
    )
...
def test_get_volumes_per_label_ok_and_with_prior():
    ...
    df_vol_prior = get_volumes_per_label(
        im_test, [0, 2, 3, 4], labels_names=["bkg", "wm", "gm", "nada"], tot_volume_prior=10,
    )
...
def test_get_volumes_per_label_sublabels():
    ...
    df_vol = get_volumes_per_label(im_test, [0, [2, 3], 4], labels_names=["bkg", "gm and wm", "nada"])
