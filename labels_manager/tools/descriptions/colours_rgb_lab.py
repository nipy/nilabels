...
def from_rgb_to_xyz(v, rgb_working_space='appleD65'):
    ...
    if rgb_working_space == 'appleD65':
        ...
        msg = 'RGB working space implemented are {0}. See website {1} for documentation.'.format(
                ['appleD65', 'sRGBD65'],
                'http://www.brucelindbloom.com/index.html?Equations.html')

        raise IOError(msg)

    return m.dot(np.array(v))


def xyz_to_lab(v, reference_white=(100.0000, 100.0000, 100.0000)):
    """
    Converting colour triplets from xyz to lab through the reference white.
    :param v: colour vector in xyz for the given reference white.
    :param reference_white:
    :return: corresponding lab colour according to the website
    http://www.brucelindbloom.com/index.html?Equations.html
    """

    # based on the CIE standard convention
    epsilon = 216 / float(24389)
    kappa = 24389 / float(27)

    v_r = [v[j] / reference_white[j] for j in range(3)]
...
def rgb_to_lab(v, rgb_working_space='appleD65', reference_white=(100.0000, 100.0000, 100.0000)):
    return xyz_to_lab(from_rgb_to_xyz(v, rgb_working_space=rgb_working_space), reference_white=reference_white)


def rgb_to_yuv(v):
    """
    From rgb triplet to YUV space. Apprximation of the rgb-lab conversion.
    :param v:
...
def yuv_to_rgb(v):
    ...
    return np.round(m_inv.dot(np.array(v))).astype('uint8')
...
def color_distance(v1, v2, in_color_space='rgb'):
    """
    Distance between v1 and v2 computed in lab-color space, where the difference is closest to the human perception.
