...
def reproduce_slice_fourth_dimension(in_data, num_slices=10, repetition_axis=3):
    ...
    new_data = np.stack([in_data, ] * num_slices, axis=repetition_axis)
    return new_data
...
def cut_4d_volume_with_a_1_slice_mask(data_4d, data_mask):
    ...
    data_masked_4d = np.zeros_like(data_dwi)
