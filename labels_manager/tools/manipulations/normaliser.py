...
def divide_by_median_below_labels_path(pfi_input, pfi_segmentation, pfi_output, thr_zeros=True):
    ...
    for label_k in list_labels:
        mask_data += im_segm.get_data() == label_k

    masked_scalar_data = (mask_data.astype(np.float64) * im_input.get_data().astype(np.float64)).flatten()
    non_zero_masked_scalar_data = masked_scalar_data[np.where(masked_scalar_data > 1e-6)]
    median = np.median(non_zero_masked_scalar_data)
